...
export default class RadioScreen extends Component {
  ...
  renderItem (checked, index, label) {
    let color = checked ? variables.hyBrandDanger : variables.hyGrayBase
    return (
      <View style={{ paddingVertical: 15 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          {index === 0 ? <Icon style={{ marginLeft: 5 }} type='question-circle' tintColor={variables.hyGrayLighter} /> : null}
        </View>
        {index === 1 ? <Text style={{ color: variables.hyGrayLighter, marginLeft: 24, fontSize: 12, marginTop: 5 }}>该选项风险较大，请谨慎选择</Text> : null}
      </View>
    )
  }
